...
async function main() {
  ...
  //获取流动性代币合约pair
  let getPair = await factory.getPair(husd.address,attach.wbnb);
  ...
  // address to,
  // uint deadline
  const removeLiquidity = await router.removeLiquidityBNB(husd.address,lp_allowance,0,0,accounts[0].address,1629969715533);
